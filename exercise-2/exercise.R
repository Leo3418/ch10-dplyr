...
cars_1997 <- mutate(cars_1997, average = (cars_1997$hwy + cars_1997$cty) / 2)

# Filter the whole vehicles data set for 2-Wheel Drive vehicles that get more
# than 20 miles/gallon in the city.
# Save this new data frame in a variable.
two_wheel_20_mpg_plus <- filter(
...
filter(two_wheel_20_mpg_plus, hwy == min(two_wheel_20_mpg_plus$hwy))$id
...
most_efficient <- function(year_choice, make_choice) {
  ...
  filter(candidates, hwy == max(candidates$hwy))
}
