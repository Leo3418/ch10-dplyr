...
two_wheel_20_mpg_plus <- vehicles[vehicles$drive == "2-Wheel Drive" &
  vehicles$cty > 20, ]

# Of the above vehicles, what is the vehicle ID of the vehicle with the worst 
# hwy mpg?
# Hint: filter for the worst vehicle, then select its ID.
two_wheel_20_mpg_plus$id[two_wheel_20_mpg_plus$hwy ==
  min(two_wheel_20_mpg_plus$hwy)]
...
most_efficient <- function(year_choice, make_choice) {
  candidates <- vehicles[vehicles$year == year_choice & vehicles$make ==
    make_choice, ]
  candidates[candidates$hwy == max(candidates$hwy), ]
}
